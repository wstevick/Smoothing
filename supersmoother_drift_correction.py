...
times1 -= times1.min()
in_peak_mask = (values1 > (peak['center'] - 3 * peak['std'])) & (values1 < peak['center'] + 3 * peak['std'])
...
for (_, peak), ax in zip(peaks1.iloc[peaks1['height'].argsort()[-10:]].iterrows(), axes.flatten()):
    in_peak_mask = (values1 > (peak['center'] - 3* peak['std'])) & (values1 < peak['center'] + 3 * peak['std'])
    in_peak_values = values1[in_peak_mask]
    in_peak_times = times1[in_peak_mask]
    in_peak_values = in_peak_values[int(.015 * len(in_peak_values)):-int(.023 * len(in_peak_values))]
    in_peak_times = in_peak_times[int(.015 * len(in_peak_times)):-int(.023 * len(in_peak_times))]
    ...
    corrections.loc[len(corrections)] = [peak['center'], y_smoothed[:int(.03*len(y_smoothed))].mean(), model]
